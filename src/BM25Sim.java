...
public class BM25Sim extends Similarity {
    ...
    public BM25Sim(float k1, float b)
    {
	this.k1 = k1;
	this.b  = b;
    }
	...
	if (termStats.length == 1) {
	    ...
	    idf = (float)Math.log(1.0D + (N - n + 0.5D) / (n + 0.5D));
	}
	else {
	...
	{
	    return ((k1 + 1) * tf) / (k1 * (K[(byte)norms.get(doc) & 0xFF]) + tf) * bw.idf; 
	}
